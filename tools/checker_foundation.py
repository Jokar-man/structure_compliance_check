...
def _get_bearing_beams(model: ifcopenshell.file, scale: float) -> list:
    """
    Find IfcBeam (or IfcMember fallback) at the lowest IfcBuildingStorey.
    Returns list of dicts: {id, name, ifc_type, storey_name, width_mm, depth_mm}
    """
    ...
    lowest_storey = min(storeys, key=_storey_elevation) if storeys else None

    # Collect beams in the lowest storey
    beams_in_storey = []
    for element_type in ("IfcBeam", "IfcMember"):
        if beams_in_storey:
            break
            ...
            try:
                if hasattr(elem, "ContainedInStructure"):
                    for rel in elem.ContainedInStructure:
                        if lowest_storey and rel.RelatingStructure == lowest_storey:
                            beams_in_storey.append(elem)
                            break
            except Exception:
                pass

    # Final fallback: all IfcBeam regardless of storey
    if not beams_in_storey:
        beams_in_storey = list(model.by_type("IfcBeam"))

    storey_name = (lowest_storey.Name or "Lowest Storey") if lowest_storey else "Unknown"
                ...
                for rep in beam.Representation.Representations:
                    for item in rep.Items:
                        if item.is_a("IfcExtrudedAreaSolid"):
                            area = item.SweptArea
                            if area.is_a("IfcRectangleProfileDef"):
                                width_m = area.XDim * scale
                                depth_m = area.YDim * scale
        except Exception:
            pass
...
def check_foundation_slab_thickness(model: ifcopenshell.file) -> list:
    """
    Art. 69 — Foundation slab minimum thickness.
    Required: 150 mm waterproof concrete + 150 mm drainage layer = 300 mm total.
    """
    scale = _get_length_scale(model)
    results = []

    candidates = list(model.by_type("IfcFooting"))
    for slab in model.by_type("IfcSlab"):
        if hasattr(slab, "PredefinedType") and slab.PredefinedType in ("BASESLAB", "FLOOR"):
    ...
    if not candidates:
        return [{
            ...
            "element_name_long": "Art. 69 — No IfcFooting or IfcSlab[BASESLAB] in model",
            "check_status":      "blocked",
            "actual_value":      None,
            "required_value":    f"{MIN_SLAB_THICKNESS_MM} mm",
            "comment":           "Model contains no foundation slab elements to check",
            "log":               None,
        }]
    ...
    for elem in candidates:
        ...
        if thickness_mm is None:
            status = "blocked"
            comment = "Thickness not found in quantity sets or extruded solid geometry"
...
def check_bearing_beam_section(model: ifcopenshell.file) -> list:
    ...
    """
    scale = _get_length_scale(model)
    beams = _get_bearing_beams(model, scale)

    if not beams:
        return [{
            "element_id":        None,
            "element_type":      "IfcBeam / IfcMember",
            "element_name":      "No bearing beams found",
            "element_name_long": "DB SE-AE — No IfcBeam or IfcMember at lowest storey",
            "check_status":      "blocked",
            "actual_value":      None,
            "required_value":    f"{MIN_BEAM_WIDTH_MM}×{MIN_BEAM_DEPTH_MM} mm",
            "comment":           "No IfcBeam or IfcMember elements found in model",
            "log":               None,
        }]
    ...
    for beam in beams:
        ...
        if w is None or d is None:
            ...
            comment = "Section dimensions not found in geometry or property sets"
        else:
            w_ok = w >= MIN_BEAM_WIDTH_MM
            d_ok = d >= MIN_BEAM_DEPTH_MM
            if w_ok and d_ok:
                status = "pass"
                comment = f"DB SE-AE satisfied: {w:.0f}×{d:.0f} mm ≥ {MIN_BEAM_WIDTH_MM}×{MIN_BEAM_DEPTH_MM} mm"
            else:
                parts = []
                if not w_ok:
                    parts.append(f"width {w:.0f} mm < {MIN_BEAM_WIDTH_MM} mm")
                if not d_ok:
                    parts.append(f"depth {d:.0f} mm < {MIN_BEAM_DEPTH_MM} mm")
                status = "fail"
                comment = "DB SE-AE: " + "; ".join(parts)
